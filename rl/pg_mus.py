...
buffer_dict = {'state_buffer':[], 'reward_buffer':[], 'action_buffer':[], 'bit_vec_buffer':[]}
...
class PolicyGradientREINFORCE(object):
    ...
    def __init__(self,# session,
        ...
        self.simulator_buffers = {}
        self.simulator_memory = {}
    ...
    def resetModel(self):
        ...
        self.exploration = self.reset_exp
        self.init_exp = self.reset_exp
    ...
    def updateModel(self, mode=dialog_config.RL_TRAINING, user_type=None):

        if self.replay:
            
            # self.memory.push(self.state_buffer, self.action_buffer, self.bit_vec_buffer, self.reward_buffer)

            state_buffer = self.simulator_buffers[user_type]['state_buffer']
            action_buffer = self.simulator_buffers[user_type]['action_buffer']
            reward_buffer = self.simulator_buffers[user_type]['reward_buffer']
            bit_vec_buffer = self.simulator_buffers[user_type]['bit_vec_buffer']

            self.simulator_memory[user_type].push(state_buffer, action_buffer, bit_vec_buffer, reward_buffer)
            self.memory = self.simulator_memory[user_type]
            print('in updateModel, memory length', user_type, len(self.memory))
            if len(self.memory) < self.batch_size:
                # clean up
                self.train_iteration += 1
                self.cleanUp(user_type=user_type)
                return
            else:
    ...
    def storeRollout(self, state, action, reward, bit_vecs=None, real_action=None, user_type=None):
        if not user_type:
            ...
            if user_type not in self.simulator_buffers:
                self.simulator_buffers[user_type] = deepcopy(buffer_dict)
                self.simulator_memory[user_type] = Memory()

            self.simulator_buffers[user_type]['action_buffer'].append(action)
            self.simulator_buffers[user_type]['reward_buffer'].append(reward)
            self.simulator_buffers[user_type]['state_buffer'].append(state)
            self.simulator_buffers[user_type]['bit_vec_buffer'].append(bit_vecs)

    def cleanUp(self, user_type=None):
        if not user_type:
            self.state_buffer  = []
            self.reward_buffer = []
            self.action_buffer = []
            ...
            self.simulator_buffers[user_type]['action_buffer'] = []
            self.simulator_buffers[user_type]['reward_buffer'] = []
            self.simulator_buffers[user_type]['state_buffer'] = []
            #self.rnn_state_buffer.append(rnn_states)
            self.simulator_buffers[user_type]['bit_vec_buffer'] = []
