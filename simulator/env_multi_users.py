...
class Enviroment(object):
    ...
    """
    def __init__(self, users, system, verbose=True, config=None):
        self.DB = DB
    ...
    def reset(self, mode, user_name = None, mturk_res=None):
        # state = env.reset()
        self.done = False
        self.success = None
        self.first_step = True
    ...
    def reset_user_dist(self, user_dist):
        print('user_dist = ', user_dist)
    ...
    def step_user(self, mode, mturk_res=None):
        ...
        if not self.config.INTERACTIVE:
            ...
            if self.config.use_new_reward:
                print('*****start to evaluate_cur_move_new*****')
    ...
    def step_system(self, provided_sys_act=None, mode=dialog_config.RL_TRAINING):
        ...
        if self.verbose:
            print("{} Sys: {}".format(self.step_i, self.last_sys_sent))
            print("True sys act: ", self.last_sys_act)
